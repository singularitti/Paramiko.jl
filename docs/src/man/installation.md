...
[Paramiko](https://github.com/singularitti/Paramiko.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("Paramiko")
   ```

3. Run

   ```julia-repl
   julia> using Paramiko
   ```
...
version of Paramiko, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/singularitti/Paramiko.jl"
```
...
our [GitHub repository](https://github.com/singularitti/Paramiko.jl)
for new releases.
Once we release a new version, you can update Paramiko by typing
